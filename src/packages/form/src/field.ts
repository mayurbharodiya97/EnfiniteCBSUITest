...
export const useField = ({
  ...
  const addRemoveFieldsFromErrorWatcher = useRecoilCallback(
    ({ set }) =>
      ({ fieldName, flag }) => {
        if (flag === "add") {
          set(
            formFieldsErrorWatcherAddSelector(formContext.formName),
            fieldName
          );
        } else if (flag === "remove") {
          set(
    ...
    (
      ...
      if (fieldDataRef.current !== null) {
        ...
        if (
          ...
          //   (eventOrTextValue as React.ChangeEvent<any>).persist();
          // }
          const { type, value, checked, options, multiple } =
  ...
  const handleOptionValueExtraData = useCallback((data) => {
    setFieldData((currVal) => {
      return { ...currVal, optionData: data };
    });
  }, []);
