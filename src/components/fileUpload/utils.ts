...
export const transformFileObject =
  (otherFieldsTemplate: any) =>
  async (file: File): Promise<FileObjectType> => {
    ...
    return {
      ...
      size: file.size,
      mimeType: file.type,
      _mimeType: mimeType?.mime ?? file.type,
      ext: mimeType?.ext ?? file.name.split(".").pop(),
...
export const convertToJsonObject = async (fileArrayObj: any[]) => {
  let FileObj: any[] = [];
  if (!Array.isArray(fileArrayObj) || fileArrayObj.length === 0) {
    return FileObj;
  }
  for (let j = 0; j < fileArrayObj.length; j++) {
    const { blob, ...others } = fileArrayObj[j];
    let base64Data = await utilFunction.convertBlobToBase64(blob);
    FileObj.push({ ...others, blob: base64Data[1], blobtype: base64Data[0] });
  }
  return FileObj;
};
