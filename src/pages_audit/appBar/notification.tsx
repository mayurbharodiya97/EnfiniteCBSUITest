...
import { Box, Chip, IconButton, Link, List, ListItemButton, Popover, Tooltip, Typography} from "@mui/material";
...
export const Notification_App = () => {
  ...
  },
  {
    ...
    message: "Our offices will be closed on April 19th in observance of the holiday.",
    type: "holiday",
    severity: "info",
  ...
  },
  {
    ...
    message: "We are experiencing a delay in processing payments. Please bear with us.",
    type: "payment",
    severity: "warning",
  ...
  },
  {
    ...
    message: "Important: We are recalling a batch of our product due to safety concerns.",
    type: "recall",
    severity: "error",
    ...
    <>
      <Tooltip title="Notification" placement="bottom" arrow>
        <IconButton
          onClick={handleClick}
          sx={{
            backgroundColor: anchorEl1 ? "var(--theme-color3)" : "rgba(235, 237, 238, 0.45)",
            borderRadius: "10px",
            height: "30px",
            width: "30px",
            "& svg": {
      ...
      <Popover
        ...
        PaperProps={{ sx: { height: 500, '::-webkit-scrollbar': { display: 'none' } } }} // Fixed height
       >
        <Box>
          <Box sx={{position: "sticky", top: 0, zIndex: 1, background:"white", p: 2 ,pb:0}}>
            <Box sx={{ display:"flex", justifyContent:"space-between"}}>
              <Typography sx={{ width: "200px" }} variant="subtitle1" noWrap fontWeight="bold">All Notifications  </Typography>
              <Link sx={{ color: "black", textDecoration: "underline" }} href="#" variant="body2">Mark all as read</Link>
            </Box>
            <Box sx={{p:1}}>
              <SelectRenderOnly
                fullWidth
          ...
          <Box sx={{ flex: "auto", overflowY: "auto",height:500 }}>
            {filteredList.map((data, index) => (
              <List disablePadding key={index}>
                <ListItemButton sx={{ padding: "0px", backgroundColor: "white" }}>
                  <Card sx={{ maxWidth:"370px",width:"100%",background: "transparent",padding:"0px" }}>
                    <CardContent sx={{ paddingBottom: '9px !important' }}>
                      <Grid container alignItems="center" justifyContent="space-between">
                        <Grid item>
                          <CardHeader sx={{ padding: 0 }}
                            avatar={<Avatar sx={{ background:'var(--theme-color3)' }}>
                              {getIcon(data.icon)}
                            </Avatar>}
                            title={
                              <Typography sx={{width:"200px"}} variant="subtitle1" noWrap fontWeight="bold">
                                {data.title}
                              </Typography>
                            }
                          />
                      ...
                      <Typography sx={{ padding: '0px 0px 0px 50px' }} variant="body2" color="text.secondary">
                        {data.message}
                      </Typography>
          ...
          <Box sx={{ flex: "none", position: "sticky", bottom: 0, zIndex: 1, p: 2, boxShadow: "rgba(0, 0, 0, 0.16) 0px 3px 6px, rgba(0, 0, 0, 0.23) 0px 3px 6px", background: "white", textAlign: "center" }}>
            <Link sx={{ color: "black" }} href="#" variant="body2">View All</Link>
          </Box>
        </Box>
      </Popover>
    </>
