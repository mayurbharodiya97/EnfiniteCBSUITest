...
const NewApplicationAccess = forwardRef<any, any>(
  ({ defaultView, username }, ref) => {
    ...
    const { userState, updateoldData, dispatchCommon } =
    ...
    const {
      ...
      error,
      refetch,
    } = useQuery<any, any>(["getnewapplicationaccess"], () => {
      if (defaultView === "new") {
    ...
    const {
      ...
      isFetching: fetching,
      refetch: Refetch,
    } = useQuery(["getapplicationaccess", Username], () => {
    ...
    useEffect(() => {
      if (
        defaultView === "edit" ||
        (defaultView === "view" && userState?.initPopulateData)
      ) {
    ...
    useEffect(() => {
      if (
        ...
        (userState?.grid1?.isUpdatedRow?.length > 0 ||
          userState?.grid1?.isNewRow?.length > 0 ||
          userState?.grid1?.isDeleteRow?.length > 0)
      ) {
        const contextGrid = [
        ...
        const updatedData = combinedData.map((item) => {
          const contextItem = contextGrid.find(
            (gridItem) => gridItem.APP_TRAN_CD === item.APP_TRAN_CD
          );
          return contextItem
