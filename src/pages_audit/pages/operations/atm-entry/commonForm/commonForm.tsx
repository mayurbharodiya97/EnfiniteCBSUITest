...
const CommonForm = (props) => {
  ...
  const deleteData: any = async ({ FLAG, DATA }) => {
    ...
    if (buttonName === "Yes") {
      ...
      crudAtmData.mutate(apiReq, {
        onSuccess(data, variables) {
          console.log("<<<delete ", data, variables);
  ...
  return (
    <>
      {validateInsert?.isLoading || atmConfirmation?.isLoading ? (
      ...
      <MasterDetailsForm
        ...
        // isError={!cardDetails?.isLoading ?? cardDetails?.isError}
        formStyle={{
          background: "white",
