...
export const GstOutwardMasterDetailForm = ({
  ...
  const getFormData = (formData) => {
    myRef.current?.setGridData((existingData) => {
      const mergedeData = [...existingData, formData];
      const dataWithUnqID = mergedeData?.map((row, index) => {
        return {
          ...row,
          TRAN_CD: index + 1,
          _isNewRow: true,
  ...
  const metaData = useMemo(() => {
    ...
    if (defaultView === "view" || defaultView === "edit") {
      myColumns = `GST Outward Entry (TRN/658) TRAN-CD: ${rows?.[0]?.data?.REF_TRAN_CD} ACCT-NM: ${rows?.[0]?.data?.ACCT_NM}`;
    } else {
      myColumns = "GST Outward Entry (TRN/658)";
    }
    return {
      ...metadata,
      masterForm: {
