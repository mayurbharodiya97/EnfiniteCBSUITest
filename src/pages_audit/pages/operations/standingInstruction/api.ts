...
export const getSearchActiveSi = async ({ companyID, branchCode, activeSiFlag }) => {

  const { data, status, message, messageDetails } = await AuthSDK.internalFetcher("GETSISEARCHBTN", {
    COMP_CD: companyID,
  ...
  if (status === "0") {
    return data.map((item) => ({
      ...
      FEQ_TYPE: item.FEQ_TYPE === "M" ? "Month(s)" : item.FEQ_TYPE === "Y" ? "Year(s)" : item.FEQ_TYPE === "Q" ? "Quartely" : item.FEQ_TYPE === "D" ? "Day(s)" : item.FEQ_TYPE === "H" ? "Half Yearly" : "",
...
export const getStandingInstructionInnerData = async ({ companyID, branchCode, Tran_cd }) => {
  const { data, status, message, messageDetails } =
...
export const getSIAsExcutedData = async ({ companyID, branchCode, acct_type, acct_cd }) => {
  const { data, status, message, messageDetails } =
...
export const getSiExecuteDetailViewData = async ({ companyID, branchCode, Tran_cd, Lien_id, Sr_cd }) => {
  const { data, status, message, messageDetails } =
...
export const AuditDataDetail = async ({ companyID, branchCode, Tran_cd, Lien_id, Sr_cd,sub_lineid,execute_date }) => {

  const { data, status, message, messageDetails } = await AuthSDK.internalFetcher("GETSIDTLAUDITBTN", {
...
export const getDebitAccountvalidation = async ({reqData}) => {
  const { data, status, message, messageDetails } =
    await AuthSDK.internalFetcher("GETDRACCTDATA", {
     ...reqData
    });
    if (status === "0") {
      return data;
...
export const getCreditAccountvalidation = async ({reqData}) => {
  const { data, status, message, messageDetails } =
    await AuthSDK.internalFetcher("GETCRACCOUNTDETAILS", {
     ...reqData
    });
    if (status === "0") {
      return data;
...
export const deleteSIExecuteDetail = async({reqdata}) =>
  ...
  export const addStandingInstructionTemplate = async ({ data: reqdata }) => {
