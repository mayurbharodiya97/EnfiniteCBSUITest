  ...
  if (MOBILE_NO) {
    ...
    if (status === "0") {
      ...
      if (message) {
        ...
        const result = await DuplicationValidate(
          columnValue,
          allField,
          formState,
          fieldValue
...
export const DuplicationValidate = async (
  ...
  if (
    ...
    Object.keys(fieldValue).includes("PASSPORT_NO")
  ) {
    if (
...
export const TrimSpaceValidation = (columnValue, allField, flag) => {
  if (columnValue.value) {
    let regex = /^[a-zA-Z ]+$/;
    if (
      columnValue.value !== columnValue.value.trimStart() &&
      columnValue.value !== columnValue.value.trimEnd()
    ) {
...
export const getOptionsOnPinParentArea = async (
  ...
  let PIN_CODE = "",
    PARENT_AREA = "";
  if (
    Boolean(dependentValue?.PIN_CODE) &&
    dependentValue?.PIN_CODE?.value?.length > 5
  ) {
...
export const getOptionsOnPinParentAreaOtherAdd = async (
  ...
  let PIN_CODE = "",
    PARENT_AREA = "";
  if (
    Boolean(dependentValue["OTHER_ADDRESS[0].PIN_CODE"]) &&
    dependentValue["OTHER_ADDRESS[0].PIN_CODE"]?.value?.length > 5
  ) {
