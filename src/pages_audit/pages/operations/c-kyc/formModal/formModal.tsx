...
const CustomTab = styled(Tab, {
  shouldForwardProp: (prop) => prop !== "isSidebarExpanded",
})<Customtabprops>(({ isSidebarExpanded, theme }) => ({
  minWidth: "60px",
  maxWidth: "250px",
    ...
    <Dialog fullScreen={true} open={state?.isFormModalOpenctx}>
      ...
      >
            ...
            >
              ...
              {false && (
                ...
                >
                  ...
                  {AccTypeOptions &&
                    AccTypeOptions.length > 0 &&
                    AccTypeOptions.map((el) => {
                      // console.log('qwewerewqdxrdsa', el)
                      return (
                        <MenuItem value={el?.DATA_VALUE}>
                          {el?.DISPLAY_VALUE}
                        </MenuItem>
                      );
                    })}
              ...
              <Grid item xs={12} sm={6} md>
                <Autocomplete
                  ...
                  // sx={{ width: 200 }}
                  renderInput={(params: any) => (
                    <TextField {...params} size="small" label="Acc. Type" />
                  )}
                  // enableGrid={false} showCheckbox={false} fieldKey={''} name={''}
                />
              </Grid>
        ...
        >
          {state?.tabsApiResctx &&
            state?.tabsApiResctx.length > 0 &&
            state?.isFreshEntryctx && <TabStepper />}
          {state?.tabsApiResctx &&
            state?.tabsApiResctx.length > 0 &&
            state?.tabsApiResctx.map((element, i) => {
              return (
