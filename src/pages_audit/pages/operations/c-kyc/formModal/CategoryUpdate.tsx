...
const CategoryUpdate = ({open, setChangeCategDialog}) => {
    ...
    const {data:categoryData, isError: isCategDTLError, isFetching: isCategDTLFetching, isLoading: isCategDTLLoading, error: CategDTLError, refetch: categDTLRefetch} = useQuery<any, any>(
      ["getCategDTL", state?.customerIDctx],
      () => API.getCategoryDTL({
    ...
    const onSubmitHandler = (
      ...
      const newData = data?.MAPPED_ACCOUNTS?.map(row => {
        ...
        const {
          ACCT_CD, ACCT_TYPE, COMP_CD, BRANCH_CD, 
          NEW_DUE_AMT, OLD_DUE_AMT,
          NEW_INST_RS, OLD_INST_RS
        } = APIDATA;
        return {
          ...others,
          ACCT_CD: ACCT_CD, ACCT_TYPE: ACCT_TYPE,
        ...
        {isCategDTLLoading ? (
            ...
            >
                {({ isSubmitting, handleSubmit }) => (
                    ...
                    <Button
                        onClick={() => {setChangeCategDialog(false)}}
                        color={"primary"}
                        disabled={isSubmitting}
                    >
                        Close
