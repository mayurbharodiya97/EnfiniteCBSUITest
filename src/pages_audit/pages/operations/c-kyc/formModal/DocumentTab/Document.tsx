...
const Document = () => {
    ...
    const onDeleteDocument = async () => {
      ...
      if(ckycState?.isFreshEntryctx) {// remove from payload - deleted main row - for fresh entry
    ...
    useEffect(() => {
      if(ckycState?.isFreshEntryctx) {
        if(Array.isArray(ckycState?.formDatactx?.["DOC_MST"]?.["DOC_MST"])) {
            ...
            changedRowsRef.current = ckycState?.formDatactx["DOC_MST"]?.["doc_mst_payload"] ?? [];
        } else {
    ...
    return (
      ...
      <TabNavigate handleSave={handleSave} displayMode={ckycState?.formmodectx ?? "new"} isNextLoading={false} />
