...
import { transformFileObject, validateFilesAndAddToList } from "components/fileUpload/utils";
import { ImageViewer, NoPreview, PDFViewer } from "components/fileUpload/preView";
...
const KYCDocumentMasterDetails = ({
    ...
    useEffect(() => {
      if(files && files.length>0) {
        console.log("fwefewqdqw", files[0]?._mimeType, files[0]?.blob, files[0]?.name)
      }
    }, [files])
    ...
    // getDocumentTypes
    const { data:DocTypes, isError, isLoading:isKYCGridDocLoading, error, refetch } = useQuery<any, any>(
      ...
      <UploadTarget
          ...
          >             */}
            {(files && files.length>0) && files[0]?._mimeType?.includes("pdf") ? (
                ...
                />
              ) : ((files && files.length>0) && files[0]?._mimeType?.includes("image")) ? (
                <ImageViewer
                  blob={files[0]?.blob}
                  fileName={files[0]?.name}
                  onClose={() => {setFiles(null)}}
