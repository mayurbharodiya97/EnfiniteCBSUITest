...
export const UserBlockConfirmGridWrapper = () => {
  ...
  const { data, isLoading, isFetching, isError, error, refetch } = useQuery<
    any,
    any
  >(["getUserBlockConfigGridData"], () => API.getUserBlockConfigGridData("N"));
  ...
  const result = useMutation(API.confirmUserBlockConfigReq, {
  ...
  const onAcceptPopupYes = (rows) => {
    result.mutate({
      confirmed: "Y",
      configType: rows[0]?.data?.CONFIG_TYPE ?? "",
    });
  };
