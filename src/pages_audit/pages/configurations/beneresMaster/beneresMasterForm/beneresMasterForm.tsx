...
import { BeneresMasterFormMetadata } from "./metaData";
...
const BeneresMasterForm: FC<{
  isDataChangedRef: any;
  closeDialog?: any;
  formView?: string;
  BeneresCode: string;
}> = ({ isDataChangedRef, closeDialog, formView, BeneresCode }) => {
  ...
  const mutation = useMutation(
    updateAUTHDetailDataWrapperFn(API.updBeneresMasterFormData),
  ...
  const masterMetadata: MetaDataType = useMemo(
    () => extractMetaData(BeneresMasterFormMetadata, formView),
    [BeneresMasterFormMetadata, formView, ""]
  ) as MetaDataType;
  return (
    <>
      <FormWrapper
        key={"BeneresMasterForm"}
...
export const BeneresMasterFormWrapper = ({
  handleDialogClose,
  isDataChangedRef,
  formView,
}) => {
  ...
  return (
    <>
      ...
      >
        <BeneresMasterForm
          BeneresCode={rows?.[0]?.data?.Beneres_CD ?? ""}
