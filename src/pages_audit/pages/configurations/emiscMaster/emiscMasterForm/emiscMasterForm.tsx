...
import { EmiscMasterFormMetadata } from "./metaData";
...
const EmiscMasterForm: FC<{
  isDataChangedRef: any;
  closeDialog?: any;
  formView?: string;
  EmiscCode: string;
}> = ({ isDataChangedRef, closeDialog, formView, EmiscCode }) => {
  ...
  const mutation = useMutation(
    updateAUTHDetailDataWrapperFn(API.updEmiscMasterFormData),
  ...
  const masterMetadata: MetaDataType = useMemo(
    () => extractMetaData(EmiscMasterFormMetadata, formView),
    [EmiscMasterFormMetadata, formView, ""]
  ) as MetaDataType;
  return (
    <>
      <FormWrapper
...
export const EmiscMasterFormWrapper = ({
  handleDialogClose,
  isDataChangedRef,
  formView,
}) => {
  ...
  return (
    <>
      ...
      >
        <EmiscMasterForm
          EmiscCode={rows?.[0]?.data?.Emisc_CD ?? ""}
